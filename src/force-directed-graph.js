...
function selectNode(selectedNode) {
    ...
    const neighbors = getNeighbors(baseLinks, selectedNode)

    // we modify the styles to highlight selected nodes
    nodeElements.attr('fill', getNodeColor(neighbors))
    textElements.attr('fill', getTextColor(neighbors))
    linkElements.attr('stroke', getLinkColor(selectedNode))
}

// this helper simple adds all nodes and links
// that are missing, to recreate the initial state
function resetData() {
    const nodeIds = nodes.map(prop('id'))
...
function updateGraph() {
    ...
    // nodes
    nodeElements = nodeGroup.selectAll('circle').data(nodes, prop('id'))
    ...
    // texts
    textElements = textGroup.selectAll('text').data(nodes, prop('id'))
